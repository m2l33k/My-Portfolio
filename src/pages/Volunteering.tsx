...
const groups: TimelineGroup[] = [
  ...
  },
  {
    ...
    type: "community",
    entries: [
      { role: "Sponsoring Manager — Comité des Élèves ESPRIT", period: "Oct 2024 – Present" },
      { role: "Active Member — Securinets Tekup", period: "Oct 2024 – Present" },
      { role: "Election Observer — ISIE (Instance Supérieure Indépendante pour les Élections)", period: "Oct 2024" },
    ],
  },
  {
...
const getRoleBadge = (role: string) => {
  ...
  if (lower.includes("chairperson") || lower.includes("director") || lower.includes("manager"))
    return { label: "Leadership", cls: "text-violet-400 bg-violet-400/10 border-violet-400/30" };
  if (lower.includes("advisor") || lower.includes("treasurer"))
    return { label: "Officer", cls: "text-cyan-400 bg-cyan-400/10 border-cyan-400/30" };
  if (lower.includes("observer"))
    return { label: "Civic", cls: "text-amber-400 bg-amber-400/10 border-amber-400/30" };
  return null;
};
          ...
          <ScrollReveal>
            <div className="text-center mb-16">
              <h2 className="text-4xl lg:text-5xl font-bold mb-4">
                <span className="text-foreground">Volunteering</span>{" "}
              ...
              <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                Community engagement, professional leadership, and civic involvement
              </p>
              ...
              <div className="flex flex-wrap justify-center gap-3 mt-8">
                {Object.entries(typeConfig).map(([key, cfg]) => (
                  <div
                    key={key}
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-full border ${cfg.border} ${cfg.bg}`}
                  >
                    <div className={`w-2 h-2 rounded-full ${cfg.dot}`} />
                    <span className={`text-xs font-medium ${cfg.color} capitalize`}>{key === "ieee" ? "IEEE" : key.replace(/([A-Z])/g, " $1")}</span>
                  </div>
                ))}
              </div>
            </div>
                  ...
                  <div className="group relative rounded-xl border border-primary/10 bg-card/50 backdrop-blur-sm overflow-hidden transition-all duration-300 hover:border-primary/20">
                    ...
                    <div className={`relative flex items-center gap-4 px-6 py-5 border-b ${cfg.border} ${cfg.bg}`}>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{group.entries.length} roles</p>
                      </div>
                    </div>
                    ...
                    <div className="relative px-6 py-4">
                      ...
                      <div className="space-y-0">
                        {group.entries.map((entry, ei) => {
                          ...
                          const roleOrg = roleParts[1] || "";

                          return (
                            <div
                              key={ei}
                              className="relative flex items-start gap-4 py-3 group/item"
                            >
                              {/* Dot */}
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex flex-wrap items-center gap-2">
                                  <p className="text-sm font-medium text-foreground/90 group-hover/item:text-foreground transition-colors">
                                    {roleTitle}
                                  </p>
                                  {badge && (
                                ...
                                {roleOrg && (
                                  <p className="text-xs text-muted-foreground/70 mt-0.5">{roleOrg}</p>
                                )}
