          ...
          <div className="space-y-6">
            {certification.imageUrl ? (
              <div className="rounded-lg overflow-hidden border border-border shadow-md bg-muted/10">
                <img src={certification.imageUrl} alt={`${certification.name} certificate`} className="w-full max-h-[300px] object-contain mx-auto" />
              </div>
            ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span>
                  {t("details.issued")} {certification.date}
                </span>
              </div>
              <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <div>
              <h4 className="font-semibold mb-2">{t("details.description")}</h4>
              <p className="text-muted-foreground">{certification.description}</p>
            </div>

            <div>
              <h4 className="font-semibold mb-2">{t("details.skillsVerified")}</h4>
            ...
            <div className="pt-4 flex justify-end">
              <Button onClick={() => window.open(certification.url, "_blank")} className="w-full sm:w-auto">
                <ExternalLink className="h-4 w-4 mr-2" />
                {t("details.viewOfficial")}
              </Button>
            </div>
          </div>
        </ScrollArea>
